...
*/
	...
	// 07ff0fbd  0000 0111 1111 1111 0000 1111 1011 1101
	/*reg32_write(SFC_BASE+SFC_BAUD, 32);
	reg32_write(SFC_BASE+SFC_CON, 0);
	reg32_write(SFC_BASE+SFC_CODE, 0);
	reg32_write(SFC_BASE+SFC_BASEADR, 0);*/

	/*hexdump((void *)0x1000000, 0x40);
	hexdump((void *)0x70200, 0x40);
	hexdump((void *)0x70300, 0x40);*/
	...
	for (;;) {
		...
		case 0x03:
			JL_uart_putc(0x06);
			static const char name[16] = "JIE LI serprog";
			for (int i = 0; i < 16; i++)
				JL_uart_putc(name[i]);
			break;
		...
		case 0x12:
			{
				uint8_t bt = JL_uart_getc();
				JL_uart_putc((bt == 0x08) ? 0x06 : 0x15);
			}
			break;
		...
		case 0x13:
			{
				int slen = JL_uart_getc()|(JL_uart_getc()<<8)|(JL_uart_getc()<<16);
				int rlen = JL_uart_getc()|(JL_uart_getc()<<8)|(JL_uart_getc()<<16);

				sflash_spi_sel(1);

				while (slen--)
					sflash_spi_send(JL_uart_getc());

				JL_uart_putc(0x06);

				while (rlen--)
					JL_uart_putc(sflash_spi_recv());
