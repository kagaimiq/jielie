...
- ...
- 64 vectored interrupts (there are two designated SFRs for that!)
...
These are some miscellaneous/core instructions and cut-down versions of their 32-bit counterparts.

The top 3 bits can be seen as the instruction 'group', where the '111' means that it's an 32-bit instruction.
So instead maybe it can be thought as the instruction width (e.g. as with C-Sky, RISC-V, NDS32, etc.)
...
These are the rest of the ISA.

The 32-bit instructions are composed of two 16-bit iwords, which can be seen as using the first iword as the top 16 bits, while the second iword fills the rest of bits.
The listing below follows this convention: the first 16 bits correspond to first iword, while the remaining ones correspond to the second iword.
